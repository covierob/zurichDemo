...
namespace Immersal
{
    ...
    public static class Core
    {
        ...
        public static icvCaptureInfo CaptureImage(byte[] capture, int captureSizeMax, byte[] pixels, int width, int height, int channels, int useMatching = 0)
        {
            ...
            icvCaptureInfo info = Native.icvCaptureImage(captureHandle.AddrOfPinnedObject(), captureSizeMax, pixelsHandle.AddrOfPinnedObject(), width, height, channels, useMatching);
            captureHandle.Free();
            pixelsHandle.Free();
        ...
        public static int LocalizeImage(out Vector3 pos, out Quaternion rot, int n, int[] handles, int width, int height, ref Vector4 intrinsics, IntPtr pixels, int param1 = 0, int param2 = 12, float param3 = 0.0f, float param4 = 2.0f, int method = 1)
        {
            GCHandle intHandle = GCHandle.Alloc(handles, GCHandleType.Pinned);
            int result = Native.icvLocalize(out pos, out rot, n, intHandle.AddrOfPinnedObject(), width, height, ref intrinsics, pixels, param1, param2, param3, param4, method);
            intHandle.Free();
            return result;
        }
        ...
        /// <param name="pixels">Raw pixel buffer data from the camera</param>
        /// <returns>An integer map ID if succeeded, -1 otherwise</returns>
        public static int LocalizeImage(out Vector3 pos, out Quaternion rot, int width, int height, ref Vector4 intrinsics, IntPtr pixels, int param1 = 0, int param2 = 12, float param3 = 0.0f, float param4 = 2.0f, int method = 1)
        {
            int n = 0;
            int[] handles = new int[1];
            return LocalizeImage(out pos, out rot, n, handles, width, height, ref intrinsics, pixels, param1, param2, param3, param4, method);
        }
    ...
    public static class Native
    {
        ...
        [DllImport(Assembly)]
        public static extern icvCaptureInfo icvCaptureImage(IntPtr capture, int captureSizeMax, IntPtr pixels, int width, int height, int channels, int useMatching);

        [DllImport(Assembly)]
        public static extern int icvLocalize(out Vector3 pos, out Quaternion rot, int n, IntPtr handles, int width, int height, ref Vector4 intrinsics, IntPtr pixels, int param1, int param2, float param3, float param4, int method);
